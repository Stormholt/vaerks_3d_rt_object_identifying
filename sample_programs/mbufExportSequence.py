...
class HookDataStruct(ctypes.Structure):
   _fields_ = [("MilSystem",MIL.MIL_ID),
                ("MilDisplay",MIL.MIL_ID),
                ("MilImageDisp",MIL.MIL_ID),
                ("MilCompressedImage",MIL.MIL_ID),
...
def ArchiveFunction( HookType,  HookId,  HookDataPtr)
   {
   ctypes.cast(ctypes.c_void_p(HookDataPtr), ctypes.POINTER(HookDataStruct)).contents
...
{    
   ...
   # Wait until we have at least 2 frames to avoid an invalid framerate. #/
   do
      {
